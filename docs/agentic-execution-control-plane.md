---
title: "Agentic Execution Control Plane"
subtitle: "A Policy-Brokered Architecture for Safe AI Agent Operations"
...
| Class | Description | Examples | Default |
|-------|-------------|----------|---------|
| 0 | Read-only | Inspect files, read logs, run git status, query non-sensitive metadata | Allow with logging |
| 1 | Local reversible write | Edit local source files, run formatter, create branch, update test fixture | Allow with logging |
| 2 | Non-production operational change | Staging migration, test environment deploy, dependency upgrade | Allow after QA and dry run |
| 3 | Production-impacting reversible change | Production deployment, feature flag change, config update | Require change linkage, rollback plan, and HITL or approved release window |
| 4 | Privileged or sensitive change | IAM change, secret rotation, DNS change, firewall rule, network exposure | Require HITL, dual control, scoped execution, and audit trail |
| 5 | Destructive or resilience-impacting action | Delete database, delete volume, delete backup, disable monitoring, remove audit logs | Deny unless routed through formal break-glass or decommission workflow |
...
1. **Specificity:** The human sees exactly what will happen.
2. **Context:** The human sees environment, target, risk, and blast radius.
3. **Evidence:** The human sees dry-run results, backup status, rollback plan, and ticket linkage.
4. **Accountability:** The approval is attributable, logged, and reviewable.
...
| Team | Owns |
|------|------|
| AI Platform Team | Agent runtime, model configuration, prompt templates, harness behavior, evaluation pipelines |
| Security / IAM Team | Credential policies, least privilege, privileged access controls, identity boundaries |
| IT Operations / Platform Engineering | Execution Broker integrations, environment boundaries, observability, runbooks |
| Enterprise Architecture | Reference architecture, capability model, control taxonomy, integration patterns |
| Risk / Compliance | Control mapping, audit evidence, approval standards, regulatory alignment |
| Application Teams | System-specific policy exceptions, deployment patterns, rollback procedures, operational readiness |

## 15. Overhead Analysis

The architecture adds overhead, but the overhead is uneven by design.

| Risk Level | Expected Overhead | Examples |
|------------|-------------------|----------|
| Low-risk local work | 1-5 seconds | Edit file, run tests, lint, format, create branch |
| Moderate-risk non-production work | 10-60 seconds | Staging migration, dependency upgrade, CI config change |
| High-risk production work | Minutes to hours | Production deploy, IAM change, database migration |
| Destructive production work | Denied by default or formal workflow | Delete database, delete backup, remove audit logs |
